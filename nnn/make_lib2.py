            ...
            for nt3 in pairs:
                for nt4 in pairs:
                    for nt5 in pairs:
                        ...
                        if 'TG' in check or 'GT' in check:
                            if check.count('GT') < 2 and check.count('TG') < 2:                        
                    ...
                    for nt5 in pairs:
                        ...
                        if 'AC' in check or 'CA' in check:
                            if check.count('AC') < 2 and check.count('CA') < 2:
                                if nt1 != 'AC' and nt1 != 'CA' and nt5 != 'AC' and nt5 != 'CA':
                                    WC3_tmp = 'G' + nt1[0] + nt2[0] + nt3[0] + nt4[0] + nt5[0] + 'G' + loop + 'C' + nt5[1] + nt4[1] + nt3[1] + nt2[1] + nt1[1] + 'C'
                                    var_lib['CA_AC'].append(WC3_tmp)
    return var_lib
...
if __name__ == '__main__':
    ...
    PK_lib = make_PKs(PKs)
    mismatch2_lib = make_mismatches2()
    ...
    series = {'triloops': triloop_lib, 'tetraloops': tetraloop_lib, \
              'bulges':bulges_lib, 'mismatches':mismatches_lib, 'triloopsNNN': triloop_libNNN, 'tetraloopsNNN': tetraloop_libNNN, \
              'bulgesNNN':bulges_libNNN, 'mismatchesNNN':mismatches_libNNN, 'WC':WC_lib, \
              'repeats':repeat, 'poly_nucleotides': polynt, 'varloop':variableloops, \
              'stem_ctrls':stem_controls, 'pseudoknots':PK_lib, 'mismatches2':mismatch2_lib, \
              'WB_controls':WB_controls, 'PUM_controls':PUM_controls}
    
    write_lib_to_file(series, 'NNNv2_4.txt')
