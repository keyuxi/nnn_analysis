...
def get_image_filename(imagedir, sequence, temperature, channel):
    ...
    channel - 'Green' or 'Red'
    """
    filename = '\"%s\\%s%02d_%g\\NNNlib2b_DNA_tileVAR_tile_%s_600ms_VAR_TIMESTAMP.tif\"' % (imagedir, channel, sequence, temperature, channel.lower())
    return filename
...
def make_folders(folderdir, seq, temperature):
    os.makedirs(os.path.join(folderdir, 'Green%02d_%g' % (seq, temperature)))
    os.makedirs(os.path.join(folderdir, 'Red%02d_%g' % (seq, temperature)))
...
def make_scripts_folders(first_melt_sequence, temperatures, n_frame, array_image_dir, template_file, script_outdir, folder_outdir):
    ...
    for seq, t in experiment_dict.items():
        ...
        outfile = os.path.join(script_outdir, '%02d_ImageBothTiles_%g.xml' % (seq, t))
        with open(outfile, 'w+') as fh:
            fh.writelines(template_content[:start_loop_ind])
            fh.writelines(movie_imaging_script_list)
            fh.writelines([s.replace('TEMPERATURE', '%g'%t) for s in template_content[end_loop_ind + 1:]])
...
if __name__ == "__main__":
    ...
    array_image_dir = 'D:\\images\\NNN-melt\\NNN_20220314'

    # The template script where variables are changed to GREEN_FILENAME, TEMPERATURE, etc.
    template_file = r'./ImageBothTiles_TEMPLATE.xml'
    # Where to put the output imaging scripts
    script_outdir = r'../data/NNN_20220314/imaging_scripts/'
    # Path to put the empty folders
    folder_outdir = r'../data/NNN_20220314/imaging_folders/'
    # =======================
